...
const updateContact = asyncHandler(async (req, res) => {
    ...
    //finding contact based upon the ID generated by MongoDB
    ...
    //find a contact document by its unique identifier (_id) and update its fields based on the data provided in req.body
    //{ new: true }: This option tells Mongoose to return the updated document after the update operation is complete.
    // Without this option, findByIdAndUpdate would return the document as it was before the update (i.e., the old document), but with { new: true }, it returns the updated document.
...
const getContact = asyncHandler(async (req, res) => {
    ...
    const contact = await Contact.findById({user_id: req.user.id}); // always await cause mongodb return a promise
    if (!contact){
        res.status(404);
        throw new Error("Contact not found");
    }
